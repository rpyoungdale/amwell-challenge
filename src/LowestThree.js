  ...
  const getLowestThreeNums = async () => {
    try {
      ...
      if(res.ok) {
        ...
        // letters. If the current element does not contain letters, it is kept
        // and added to the dataElements array as an integer. If it does contain
        // letters, it is not added to the array.
        let manipulatedData = rawDataArray.flatMap((elem) => {
            return !isNaN(parseInt(elem)) ? [parseInt(elem)] : [];
        })
  
        manipulatedData.sort((a, b) => a - b);
        ...
        manipulatedData = [...new Set(manipulatedData)];
  
        setLowestThree(manipulatedData.slice(0, 3));
      } else {
